...
inline bool toMsg(tesseract_msgs::msg::Mesh& mesh_msgs, const tesseract::geometry::Geometry& geometry)
{
...
std::vector<Eigen::Vector3d> sdfSurfaceShellPoints(const tesseract::geometry::SignedDistanceField& sdf, double band)
{
  ...
  // The loops visit samples in storage order (i fastest), so a running counter indexes the
  // flat array without widening int index arithmetic that could overflow on a large grid.
  std::size_t idx = 0;
  for (int k = 0; k < nz; ++k)
  {
    for (int j = 0; j < ny; ++j)
    {
      for (int i = 0; i < nx; ++i, ++idx)
      {
        ...
        if (std::abs(d) > band)
          continue;
        points.emplace_back(
            domain.min().x() + i * spacing.x(), domain.min().y() + j * spacing.y(), domain.min().z() + k * spacing.z());
      }
    }
  }
  return points;
...
bool toMsg(tesseract_msgs::msg::Geometry& geometry_msgs, const tesseract::geometry::Geometry& geometry)
{
...
inline std::shared_ptr<tesseract::geometry::PolygonMesh> fromMsg(uint8_t type,
                                                                 const tesseract_msgs::msg::Mesh& mesh_msg)
{
  ...
  if (type == tesseract_msgs::msg::Geometry::POLYGON_MESH)
  {
    ...
    if (!mesh_msg.file_path.empty())
      return std::make_shared<tesseract::geometry::PolygonMesh>(
          vertices,
          faces,
          std::make_shared<tesseract::common::SimpleLocatedResource>(mesh_msg.file_path, mesh_msg.file_path),
          Eigen::Vector3f(mesh_msg.scale[0], mesh_msg.scale[1], mesh_msg.scale[2]).cast<double>());

    return std::make_shared<tesseract::geometry::PolygonMesh>(vertices, faces);
  }

  return nullptr;
}

bool fromMsg(tesseract::geometry::Geometry::Ptr& geometry, const tesseract_msgs::msg::Geometry& geometry_msg)
{
  geometry = nullptr;
  ...
  else if (geometry_msg.type == tesseract_msgs::msg::Geometry::SIGNED_DISTANCE_FIELD)
  {
    ...
    const bool has_vdb = !sdf_msg.vdb.empty();
    const bool has_nvdb = !sdf_msg.nvdb.empty();
    if (has_vdb && has_nvdb)
      throw std::runtime_error("fromMsg: SignedDistanceField message has both a vdb and an nvdb payload; "
...
bool toMsg(tesseract_msgs::msg::Material& material_msg, const tesseract::scene_graph::Material::Ptr& material)
{
  if (material == nullptr)
  {
...
bool fromMsg(tesseract::scene_graph::Material::Ptr& material, const tesseract_msgs::msg::Material& material_msg)
{
...
bool toMsg(tesseract_msgs::msg::Inertial& inertial_msg, const tesseract::scene_graph::Inertial::Ptr& inertial)
{
  if (inertial == nullptr)
  {
...
bool fromMsg(tesseract::scene_graph::Inertial::Ptr& inertial, const tesseract_msgs::msg::Inertial& inertial_msg)
{
...
bool toMsg(tesseract_msgs::msg::VisualGeometry& visual_msg, const tesseract::scene_graph::Visual& visual)
{
...
bool fromMsg(tesseract::scene_graph::Visual::Ptr& visual, const tesseract_msgs::msg::VisualGeometry& visual_msg)
{
...
bool toMsg(tesseract_msgs::msg::CollisionGeometry& collision_msg, const tesseract::scene_graph::Collision& collision)
{
...
bool fromMsg(tesseract::scene_graph::Collision::Ptr& collision,
...
bool toMsg(tesseract_msgs::msg::Link& link_msg, const tesseract::scene_graph::Link& link)
{
...
tesseract::scene_graph::Link fromMsg(const tesseract_msgs::msg::Link& link_msg)
{
  tesseract::scene_graph::Link link(tesseract::common::LinkId(link_msg.name));
...
bool toMsg(tesseract_msgs::msg::JointCalibration& joint_calibration_msg,
           const tesseract::scene_graph::JointCalibration::Ptr& joint_calibration)
{
...
bool fromMsg(tesseract::scene_graph::JointCalibration::Ptr& joint_calibration,
...
bool toMsg(tesseract_msgs::msg::JointDynamics& joint_dynamics_msg,
           const tesseract::scene_graph::JointDynamics::Ptr& joint_dynamics)
{
...
bool fromMsg(tesseract::scene_graph::JointDynamics::Ptr& joint_dynamics,
...
bool toMsg(tesseract_msgs::msg::JointLimits& joint_limits_msg,
           const tesseract::scene_graph::JointLimits::Ptr& joint_limits)
{
...
bool fromMsg(tesseract::scene_graph::JointLimits::Ptr& joint_limits,
...
bool toMsg(tesseract_msgs::msg::JointMimic& joint_mimic_msg, const tesseract::scene_graph::JointMimic::Ptr& joint_mimic)
{
...
bool fromMsg(tesseract::scene_graph::JointMimic::Ptr& joint_mimic,
...
bool toMsg(tesseract_msgs::msg::JointSafety& joint_safety_msg,
           const tesseract::scene_graph::JointSafety::Ptr& joint_safety)
{
...
bool fromMsg(tesseract::scene_graph::JointSafety::Ptr& joint_safety,
...
bool toMsg(tesseract_msgs::msg::Joint& joint_msg, const tesseract::scene_graph::Joint& joint)
{
...
tesseract::scene_graph::Joint fromMsg(const tesseract_msgs::msg::Joint& joint_msg)
{
  tesseract::scene_graph::Joint joint(tesseract::common::JointId(joint_msg.name));
  ...
  joint.child_link_id = tesseract::common::LinkId(joint_msg.child_link_name);
  joint.parent_link_id = tesseract::common::LinkId(joint_msg.parent_link_name);
...
fromMsg(const std::vector<tesseract_msgs::msg::ContactMarginPair>& contact_margin_pairs_msg)
{
  ...
  for (const auto& pair : contact_margin_pairs_msg)
  {
    ...
    if (id1.value() <= id2.value())
      contact_margin_pairs.emplace(
          id_pair, tesseract::common::PairMarginEntry{ pair.first.first, pair.first.second, pair.second });
    else
      contact_margin_pairs.emplace(
          id_pair, tesseract::common::PairMarginEntry{ pair.first.second, pair.first.first, pair.second });
  }
...
toMsg(const tesseract::common::PairsCollisionMarginData& contact_margin_pairs)
{
  ...
  for (const auto& [key, entry] : contact_margin_pairs)
  {
    ...
    cmp.first.first = entry.name1;
    cmp.first.second = entry.name2;
...
tesseract::common::CollisionMarginData fromMsg(const tesseract_msgs::msg::CollisionMarginData& contact_margin_data_msg)
{
...
tesseract_msgs::msg::CollisionMarginData toMsg(const tesseract::common::CollisionMarginData& contact_margin_data)
{
...
void toMsg(tesseract_msgs::msg::SceneGraph& scene_graph_msg, const tesseract::scene_graph::SceneGraph& scene_graph)
{
  scene_graph_msg.id = scene_graph.getName();
  scene_graph_msg.root = scene_graph.getRoot().name();
  ...
  for (const auto& link : scene_graph.getLinks())
  {
    ...
    if (!scene_graph.getLinkVisibility(link->getId()))
      scene_graph_msg.invisible_links.push_back(link->getName());

    if (!scene_graph.getLinkCollisionEnabled(link->getId()))
...
tesseract::scene_graph::SceneGraph fromMsg(const tesseract_msgs::msg::SceneGraph& scene_graph_msg)
{
  ...
  g.setRoot(tesseract::common::LinkId(scene_graph_msg.root));

  for (const auto& link_name : scene_graph_msg.invisible_links)
    g.setLinkVisibility(tesseract::common::LinkId(link_name), false);

  for (const auto& link_name : scene_graph_msg.disabled_collision_links)
    g.setLinkCollisionEnabled(tesseract::common::LinkId(link_name), false);

  for (const auto& entry : scene_graph_msg.acm)
    g.getAllowedCollisionMatrix()->addAllowedCollision(
        tesseract::common::LinkId(entry.link_1), tesseract::common::LinkId(entry.link_2), entry.reason);

  return g;
}

bool toMsg(tesseract_msgs::msg::EnvironmentCommand& command_msg, const tesseract::environment::Command& command)
{
  switch (command.getType())
  {
    ...
    case tesseract::environment::CommandType::MOVE_JOINT:
    {
      ...
      command_msg.move_joint_name = cmd.getJointId().name();
      command_msg.move_joint_parent_link = cmd.getParentLink().name();
      return true;
    }
    case tesseract::environment::CommandType::REMOVE_LINK:
    {
      ...
      command_msg.remove_link = cmd.getLinkId().name();
      return true;
    }
    case tesseract::environment::CommandType::REMOVE_JOINT:
    {
      ...
      command_msg.remove_joint = cmd.getJointId().name();
      return true;
    }
    ...
    case tesseract::environment::CommandType::CHANGE_JOINT_ORIGIN:
    {
      ...
      command_msg.change_joint_origin_name = cmd.getJointId().name();
    ...
    case tesseract::environment::CommandType::CHANGE_LINK_COLLISION_ENABLED:
    {
      ...
      command_msg.change_link_collision_enabled_name = cmd.getLinkId().name();
    ...
    case tesseract::environment::CommandType::CHANGE_LINK_VISIBILITY:
    {
      ...
      command_msg.change_link_visibility_name = cmd.getLinkId().name();
    ...
    case tesseract::environment::CommandType::REMOVE_ALLOWED_COLLISION_LINK:
    {
      ...
      command_msg.remove_allowed_collision_link = cmd.getLinkId().name();
      return true;
    }
    ...
    case tesseract::environment::CommandType::ADD_TRAJECTORY_LINK:
    {
      ...
      command_msg.add_trajectory_link_name = cmd.getLinkId().name();
      command_msg.add_trajectory_link_parent_name = cmd.getParentLinkId().name();
...
tesseract::environment::Commands fromMsg(const std::vector<tesseract_msgs::msg::EnvironmentCommand>& commands_msg)
{
...
tesseract::environment::Command::Ptr fromMsg(const tesseract_msgs::msg::EnvironmentCommand& command_msg)
{
  switch (command_msg.command)
  {
    case tesseract_msgs::msg::EnvironmentCommand::ADD_LINK:
    {
      tesseract::scene_graph::Link link = fromMsg(command_msg.add_link);
      if (command_msg.add_joint.name.empty() || command_msg.add_joint.type == 0)
        return std::make_shared<tesseract::environment::AddLinkCommand>(link, command_msg.add_replace_allowed);

      tesseract::scene_graph::Joint joint = fromMsg(command_msg.add_joint);
      return std::make_shared<tesseract::environment::AddLinkCommand>(link, joint, command_msg.add_replace_allowed);
    }
    ...
    {
      return std::make_shared<tesseract::environment::MoveJointCommand>(
          tesseract::common::JointId(command_msg.move_joint_name),
          tesseract::common::LinkId(command_msg.move_joint_parent_link));
    }
    case tesseract_msgs::msg::EnvironmentCommand::REMOVE_LINK:
    {
      return std::make_shared<tesseract::environment::RemoveLinkCommand>(
          tesseract::common::LinkId(command_msg.remove_link));
    }
    case tesseract_msgs::msg::EnvironmentCommand::REMOVE_JOINT:
    {
      return std::make_shared<tesseract::environment::RemoveJointCommand>(
          tesseract::common::JointId(command_msg.remove_joint));
    }
    ...
    case tesseract_msgs::msg::EnvironmentCommand::CHANGE_JOINT_ORIGIN:
    {
      ...
      return std::make_shared<tesseract::environment::ChangeJointOriginCommand>(
          tesseract::common::JointId(command_msg.change_joint_origin_name), pose);
    }
    case tesseract_msgs::msg::EnvironmentCommand::CHANGE_LINK_COLLISION_ENABLED:
    {
      return std::make_shared<tesseract::environment::ChangeLinkCollisionEnabledCommand>(
          tesseract::common::LinkId(command_msg.change_link_collision_enabled_name),
          command_msg.change_link_collision_enabled_value);
    }
    ...
    case tesseract_msgs::msg::EnvironmentCommand::REMOVE_ALLOWED_COLLISION_LINK:
    {
      return std::make_shared<tesseract::environment::RemoveAllowedCollisionLinkCommand>(
          tesseract::common::LinkId(command_msg.remove_allowed_collision_link));
    }
    ...
    {
      std::unordered_map<tesseract::common::JointId, std::pair<double, double>> limits_map;
      for (const auto& limits : command_msg.change_joint_position_limits)
        limits_map[tesseract::common::JointId(limits.first)] = std::make_pair(limits.second[0], limits.second[1]);
    ...
    {
      std::unordered_map<tesseract::common::JointId, double> limits_map;
      for (const auto& limits : command_msg.change_joint_velocity_limits)
        limits_map[tesseract::common::JointId(limits.first)] = limits.second;
    ...
    case tesseract_msgs::msg::EnvironmentCommand::ADD_TRAJECTORY_LINK:
    {
      ...
      return std::make_shared<tesseract::environment::AddTrajectoryLinkCommand>(
          tesseract::common::LinkId(command_msg.add_trajectory_link_name),
          tesseract::common::LinkId(command_msg.add_trajectory_link_parent_name),
          traj,
...
void toMsg(tesseract_msgs::msg::JointTrajectory& traj_msg, const tesseract::common::JointTrajectory& traj)
{
...
tesseract::common::JointTrajectory fromMsg(const tesseract_msgs::msg::JointTrajectory& traj_msg)
{
  ...
  for (const auto& js_msg : traj_msg.states)
  {
    ...
    js.joint_ids = tesseract::common::toIds<tesseract::common::JointId>(js_msg.joint_names);
...
tesseract_msgs::msg::KinematicsPluginInfo toMsg(const tesseract::common::KinematicsPluginInfo& info)
{
...
tesseract_msgs::msg::ContactManagersPluginInfo toMsg(const tesseract::common::ContactManagersPluginInfo& info)
{
...
std::vector<tesseract_msgs::msg::StringPluginInfoPair> toMsg(const tesseract::common::PluginInfoMap& info_map)
{
...
tesseract_msgs::msg::PluginInfo toMsg(const tesseract::common::PluginInfo& info)
{
...
bool toMsg(geometry_msgs::msg::PoseArray& pose_array, const tesseract::common::VectorIsometry3d& transforms)
{
...
tesseract_msgs::msg::ChainGroup toMsg(tesseract::srdf::ChainGroups::const_reference group)
{
  ...
  for (const auto& pair : group.second)
  {
    ...
    chain.first = pair.first.name();
    chain.second = pair.second.name();
    g.chains.push_back(chain);
  }
  return g;
}

tesseract_msgs::msg::GroupsJointStates toMsg(tesseract::srdf::GroupJointStates::const_reference group)
{
...
tesseract_msgs::msg::GroupsTCPs toMsg(tesseract::srdf::GroupTCPs::const_reference group)
{
...
bool toMsg(tesseract_msgs::msg::KinematicsInformation& kin_info_msg,
           const tesseract::srdf::KinematicsInformation& kin_info)
{
  ...
  for (const auto& group : kin_info.chain_groups)
  {
    ...
    for (const auto& chain : group.second)
    {
      ...
      pair.first = chain.first.name();
      pair.second = chain.second.name();
             ...
             const tesseract_msgs::msg::KinematicsInformation& kin_info_msg)
{
  ...
  for (const auto& group : kin_info_msg.group_joint_states)
  {
    for (const auto& state : group.joint_states)
    {
      ...
      for (const auto& js : state.joint_state)
        joint_state[tesseract::common::JointId(js.first)] = js.second;
  ...
  for (const auto& group : kin_info_msg.group_tcps)
  {
    for (const auto& pose : group.tcps)
    {
      ...
      kin_info.group_tcps[group.name][tesseract::common::LinkId(pose.name)] = tcp;
    }
  }
...
tesseract::common::KinematicsPluginInfo fromMsg(const tesseract_msgs::msg::KinematicsPluginInfo& info_msg)
{
...
tesseract::common::ContactManagersPluginInfo fromMsg(const tesseract_msgs::msg::ContactManagersPluginInfo& info_msg)
{
...
tesseract::common::PluginInfoMap fromMsg(const std::vector<tesseract_msgs::msg::StringPluginInfoPair>& info_map_msg)
{
           ...
           const tesseract::common::JointIdTransformMap& transform_map)
{
  ...
  {
    transform_map_msg.names.push_back(pair.first.name());
    geometry_msgs::msg::Pose pose;
...
bool toMsg(tesseract_msgs::msg::Environment& environment_msg,
           const tesseract::environment::Environment::ConstPtr& env,
           bool include_joint_states)
{
  return toMsg(environment_msg, *env, include_joint_states);
}

tesseract::environment::Environment::UPtr fromMsg(const tesseract_msgs::msg::Environment& environment_msg)
{
  tesseract::environment::Commands commands;
  try
  {
...
bool toMsg(tesseract_msgs::msg::TaskComposerNodeInfo& node_info_msg,
           const tesseract::task_composer::TaskComposerNodeInfo& node_info)
{
...
tesseract::task_composer::TaskComposerNodeInfo fromMsg(const tesseract_msgs::msg::TaskComposerNodeInfo& node_info_msg)
{
...
trajectory_msgs::msg::JointTrajectory toMsg(const tesseract::common::JointTrajectory& joint_trajectory,
...
tesseract::common::JointTrajectory fromMsg(const trajectory_msgs::msg::JointTrajectory& joint_trajectory_msg)
{
  ...
  for (const auto& state_msg : joint_trajectory_msg.points)
  {
    ...
    state.joint_ids = tesseract::common::toIds<tesseract::common::JointId>(joint_trajectory_msg.joint_names);
...
void toTransformMsgs(const std::shared_ptr<tesseract::environment::Environment>& env,
                     const rclcpp::Time& stamp,
