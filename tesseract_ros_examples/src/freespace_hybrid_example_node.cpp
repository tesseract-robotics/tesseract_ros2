...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("freespace_hybrid_example_node");
  ...
  if (!example.run())
  {
    RCLCPP_ERROR(node->get_logger(), "FreespaceHybridExample failed");
  }
  else
  {
    RCLCPP_INFO(node->get_logger(), "FreespaceHybridExample successful");
  }
