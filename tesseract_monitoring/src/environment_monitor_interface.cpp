...
typename SrvType::Response::SharedPtr call_service(
...
bool EnvironmentMonitorInterface::waitForNamespace(const std::string& monitor_namespace, rclcpp::Duration timeout) const
{
  ...
  try
  {
    auto res = call_service<tesseract_msgs::srv::GetEnvironmentInformation>(
        service_name, req, *node_, callback_group_, 5s);
    if (! res->success)
    {
                                               ...
                                               const tesseract_environment::Command& command) const
{
  ...
  {
    RCLCPP_ERROR_STREAM(
      node_->get_logger().get_child("environment_monitor_interface").get_child(monitor_namespace),
...
EnvironmentMonitorInterface::getEnvironmentState(const std::string& monitor_namespace) const
{
  ...
  try
  {
    ...
    auto res = call_service<tesseract_msgs::srv::GetEnvironmentInformation>(
      srv_name, req, *node_, callback_group_, 3s);
