...
ROSEnvironmentMonitorInterface::applyCommand(const tesseract::environment::Command& command) const
{
...
std::vector<std::string>
ROSEnvironmentMonitorInterface::applyCommands(const tesseract::environment::Commands& commands) const
{
  std::vector<std::string> failed_namespace;
  failed_namespace.reserve(ns_.size());
  for (const auto& ns : ns_)
...
std::vector<std::string>
ROSEnvironmentMonitorInterface::applyCommands(const std::vector<tesseract::environment::Command>& commands) const
{
  std::vector<std::string> failed_namespace;
  failed_namespace.reserve(ns_.size());
  for (const auto& ns : ns_)
...
bool ROSEnvironmentMonitorInterface::applyCommand(const std::string& monitor_namespace,
                                                  const tesseract::environment::Command& command) const
{
...
bool ROSEnvironmentMonitorInterface::applyCommands(const std::string& monitor_namespace,
                                                   const tesseract::environment::Commands& commands) const
{
...
bool ROSEnvironmentMonitorInterface::applyCommands(const std::string& monitor_namespace,
                                                   const std::vector<tesseract::environment::Command>& commands) const
{
  std::vector<tesseract_msgs::msg::EnvironmentCommand> commands_msg;
    ...
    const tesseract::common::JointIdTransformMap& floating_joints) const
{
  ...
  tesseract_rosutils::toMsg(command.joint_state, joints_str);
...
std::vector<std::string>
ROSEnvironmentMonitorInterface::setEnvironmentState(const tesseract::scene_graph::SceneState::JointValues& joints,
                                                    const tesseract::common::JointIdTransformMap& floating_joints) const
{
  std::vector<std::string> failed_namespace;
  failed_namespace.reserve(ns_.size());
...
std::vector<std::string>
ROSEnvironmentMonitorInterface::setEnvironmentState(const std::vector<tesseract::common::JointId>& joint_ids,
                                                    const Eigen::Ref<const Eigen::VectorXd>& joint_values,
                                                    const tesseract::common::JointIdTransformMap& floating_joints) const
{
  std::vector<std::string> failed_namespace;
  failed_namespace.reserve(ns_.size());
  for (const auto& ns : ns_)
    if (!setEnvironmentState(ns, joint_ids, joint_values, floating_joints))
      failed_namespace.push_back(ns);
...
std::vector<std::string>
ROSEnvironmentMonitorInterface::setEnvironmentState(const tesseract::common::JointIdTransformMap& floating_joints) const
{
  std::vector<std::string> failed_namespace;
  failed_namespace.reserve(ns_.size());
  for (const auto& ns : ns_)
