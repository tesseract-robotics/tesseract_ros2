...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<rclcpp::Node>("pick_and_place_example_node");
  ...
  if (!example.run())
  {
    RCLCPP_ERROR(node->get_logger(), "PickAndPlaceExample failed");
  }
  else
  {
    RCLCPP_INFO(node->get_logger(), "PickAndPlaceExample successful");
  }
